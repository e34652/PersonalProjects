...
// 상품 테스트	
	...
	@DisplayName("상품등록")
	void insertProduct() {
		Product product = Product.builder().name("abc").genre("Rhythm").releaseDate("추후 출시 예정").price(9999).simple("리듬게임").detail("재미있다").build();
	...
	@DisplayName("상품갱신")
	void updateProduct() {
		Product product = Product.builder().num(3).name("abc123").genre("Rhythm").releaseDate("추후 출시 예정").price(9999).simple("리듬게임").detail("재미있다").build();
...
//	@DisplayName("회원등록")
//	void insertMember() {
//		Member member = Member.builder().authority(0).email("1666@123").nationality("한국").nickname("문곰문곰문곰1").password("123").build();
//		
...
//	@DisplayName("회원갱신")
//	void updateMember() {
//		Member member = Member.builder().num(5).authority(0).email("144@123").nationality("한국국").nickname("문곰문곰문곰2").password("123").build();
...
//		log.info("member: {}", mMapper.selectMemberByNum(3));
//	}
//	@Test
//	@Order(5)
//	@DisplayName("회원삭제")
